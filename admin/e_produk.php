...
if (isset($_POST['update'])) {
    ...
    if ($_FILES['gambar']['name'] != '') {
        ...
        $extension = strtolower(pathinfo($img_file, PATHINFO_EXTENSION));
        $dir = "produk_img/";
        ...
        if (in_array($extension, $allowed_extensions)) {
            ...
            $imgnewfile = md5(time() . $img_file) . '.' . $extension;
            move_uploaded_file($tmp_file, $dir . $imgnewfile);

        }
    
    } else {
        $imgnewfile = $gambar_lama; // gunakan gambar lama jika tidak ada gambar baru
    }
